...
public class Fonction
{
    ...
    public static void Action(int idprojet, ApplicationDbContext _context)
    {
        ...
        List<OccurenceActivite> listeAC = _context.OccurenceActivite
            .ToList();
        foreach (var v in listeAC)
        {
            if(v.Avancement >= 100)
            {
                VDateDebutRealisationActivite vd = _context.VDateDebutRealisationActivite
                    .First(a => a.IdOccurenceActivite == v.Id);
                VDateFinRealisationActivite vf = _context.VDateFinRealisationActivite
                    .First(a => a.IdOccurenceActivite == v.Id);
                DateRealisationActivite dr = new DateRealisationActivite()
                {
                    IdOccurenceActivite = v.Id,
                    DateDebutRealisation = vd.DateDebut,
                    DateFinRealisation = vf.DateFin
                };
                _context.Add(dr);
                _context.SaveChanges();
            }
    ...
    public static Mois MoisById(int idmois, ApplicationDbContext _context)
    {
        return _context.Mois.First(a => a.Id == idmois);
    }
}